...
function App() {
  ...
  const [viewingSubmission, setViewingSubmission] = useS(null); // when viewing a past submission read-only
  ...
  useE(() => {
    ...
    // Re-run if not seeded yet OR if the flag version changed
    ...
    SEED_EMAILS.forEach((email) => {
      ...
      const submissions = [
        {
          ...
          ts: Date.now() - 1000 * 60 * 60 * 24 * 12, // ~12 days ago
      ...
      // Register the account so login "recognizes" them
      try {
  ...
  // Theme
  useE(() => {
    const theme = window.THEMES[config.theme] || window.THEMES.notebook;
  ...
  const flat = window.flattenQuestions(window.SECTIONS, answers);
  ...
  const goWelcome = () => setPhase(PHASES.WELCOME);
  const startApp = () => setPhase(PHASES.AUTH);

  const onAuthed = (u) => {
    setUser(u);
    // On login, ALWAYS show the returning-user choice screen.
    // On register (first-time account), go straight into upload.
    if (u.mode === "login") {
  ...
  const onResumeDraft = () => {
    // Jump back to where they were; default to SECTION_INTRO if no stepIdx
  ...
  const onViewPastSubmission = (sub) => {
    // Load their old answers into read-only view by jumping to DONE with that data
  ...
  const onParseDone = () => {
    const p = window.simulateParse(user?.email);
    setParsed(p);
    setPhase(PHASES.REVIEW);
  };

  const onReviewDone = (fields) => {
    // Pre-fill the basic info answers from parsed fields
    const next = { ...answers };
  ...
  const logout = () => {
    // Keep drafts and submissions on disk — just clear the session
    ["tir:phase","tir:user"].forEach(k => localStorage.removeItem(k));
    setUser(null);
    setPhase(PHASES.WELCOME);
  };

  const goProfile = () => setPhase(PHASES.PROFILE);
  ...
  const lock = window.useSessionLock({
    sharedAppEmails,
  ...
  useE(() => {
    const handler = (e) => {
      ...
      if (e.target.tagName === "INPUT" && phase === PHASES.AUTH) return; // auth handles its own
      if (e.key === "Enter" && !e.shiftKey) {
        ...
        if (phase === PHASES.QUESTION && currentFQ) {
          ...
          if (window.isAnswered(currentFQ.q, v)) {
            e.preventDefault();
            goNextQuestion();
          }
        }
  ...
  // Progress calculation — across all of flat
  ...
  const currentSection = currentFQ?.section || window.SECTIONS[sectionIdx];
    ...
    <div className={`eir-root eir-theme-${config.theme}`}>
      ...
      <div className="eir-frame">
        ...
        <main className="eir-main">
          ...
          {phase === PHASES.SECTION_INTRO && <SectionIntroScreen section={window.SECTIONS[sectionIdx]} onContinue={() => setPhase(PHASES.QUESTION)} />}
...
function QuestionView({ fq, total, stepIdx, value, onChange, onNext, onPrev, canPrev, warmCopy, answers }) {
  ...
  const answered = window.isAnswered(q, value);
  const blockReason = answered ? null : (window.whyBlocked ? window.whyBlocked(q, value) : null);
  const name = (answers.fullName || "").split(" ")[0];

  // Personalize a few prompts
  let prompt = q.prompt;
  if (warmCopy && name) {
