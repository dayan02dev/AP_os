...
function DoneScreen({ answers, onRestart, submission, onBack }) {
  ...
  const progress = isPast && window.getSubmissionProgress ? window.getSubmissionProgress(submission) : null;
  const MILESTONES = window.MILESTONES || [];
  const statusLabel = isPast && window.getStatusLabel ? window.getStatusLabel(submission) : "submitted";
    ...
    <div className="eir-screen eir-done">
      ...
      <div className="eir-done-body">
        ...
        <div className="eir-q-actions">
          {isPast ? (
            <button className="eir-btn eir-btn-primary" onClick={onBack}>
...
Object.assign(window, { ProgressBar, WelcomeScreen, SectionIntroScreen, CelebrationScreen, DoneScreen });
