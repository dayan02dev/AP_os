...
function EmailInput({ q, value, onChange, autoFocus }) {
  const ref = useRef(null);
  useEffect(() => { if (autoFocus && ref.current) ref.current.focus(); }, [autoFocus]);
  return (
    <input
...
function LongInput({ q, value, onChange, autoFocus }) {
  ...
  // Soft "sweet spot" guidance — 50% of the schema's old minWords, or a sensible default.
...
function TeamInviteInput({ q, value, onChange }) {
  ...
  const [draftOpenIdx, setDraftOpenIdx] = useState(null); // which member card is expanded
  ...
  const sendInvite = (idx) => {
    const m = members[idx];
    const emailCheck = window.validateEmail ? window.validateEmail(m.email) : { valid: /\S+@\S+\.\S+/.test(m.email || "") };
    if (!emailCheck.valid) {
  ...
  const memberComplete = (m) => !!(m && m.email && m.fullName && m.phone && m.org && (window.validateEmail ? window.validateEmail(m.email).valid : true));
                  ...
                  <div className="eir-team-card-body">
                    <div className="eir-team-field">
                      ...
                      {window.EmailInput ? (
                        ...
                        <input className="eir-input" type="email" value={m.email || ""} onChange={(e) => updateMember(idx, { email: e.target.value })} placeholder="cofounder@domain.com" />
                      )}
                    </div>
                    <div className="eir-team-field">
...
function isAnswered(q, value) {
  ...
  switch (q.kind) {
    ...
    case "teamInvite":
      // Require at least one fully-filled teammate
...
// Returns a human-readable reason why the answer is blocked, or null if OK.
...
Object.assign(window, { QuestionInput, isAnswered, whyBlocked });
