-- NOTE (carried onto feat/mou-agreements-resource-lock 2026-08-19):
-- This file originates on feat/vip-onboarding. It is duplicated here, byte
-- for byte, because THIS branch's code depends on it: founder_mou.sign
-- writes a `track` value, so a schema built from this branch's migrations
-- alone would be missing the column and every signature would fail.
-- Migrations 044 and 045 are NOT copied — they create VIP-only AIR/MIS
-- tables that no code on this branch touches. The numbering gap is
-- deliberate, not an omission.
--
-- Already applied to production on 2026-08-18 and verified: all five
-- founder_* tables carry `track`, and the single existing MOU row
-- backfilled to 'tir'.
