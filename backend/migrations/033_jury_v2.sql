...
-- 1. Allow 'jury' in user_roles role CHECK (constraint auto-named in mig 014).
do $$
declare cname text;
begin
...
-- 2. admin_decisions CHECK: keep prod's jury_review (mig 027) AND add offered.
do $$
declare cname text;
begin
  select conname into cname from pg_constraint
   where conrelid = 'public.admin_decisions'::regclass and contype = 'c'
     and pg_get_constraintdef(oid) ilike '%decision%in%';
  if cname is not null then
    execute format('alter table public.admin_decisions drop constraint %I', cname);
  end if;
