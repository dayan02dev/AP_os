...
from app.routers import milestone_files as mf
...
def fakes(monkeypatch):
    ...
    monkeypatch.setattr(mf, "get_admin_client", lambda: _Client())
    monkeypatch.setattr(mf.submitted_edit, "load_editable_app", lambda *a, **k: dict(state["row"]))
    monkeypatch.setattr(mf.submitted_edit, "mark_edited", lambda t, i, tr: state["marked"].append((i, tr)))
    return state


def test_upload_to_submitted_app_marks_edited(client, fakes):
    res = client.post(
        f"/applications/me/milestone-files?application_id={APP_ID}",
...
def test_upload_window_closed_403(client, fakes, monkeypatch):
    ...
    monkeypatch.setattr(mf.submitted_edit, "load_editable_app", _raise)
    res = client.post(
        f"/applications/me/milestone-files?application_id={APP_ID}",
        files={"file": ("m.pdf", io.BytesIO(b"%PDF-1.4 data"), "application/pdf")},
    )
