...
from app.routers import sip_milestone_files as smf
...
def fakes(monkeypatch):
    state = {"row": {"id": APP_ID, "user_id": USER, "status": "submitted", "execution_milestone_files": []},
             "marked": []}
    ...
    monkeypatch.setattr(smf, "get_admin_client", lambda: _Client())
    monkeypatch.setattr(smf.submitted_edit, "load_editable_app", lambda *a, **k: dict(state["row"]))
    monkeypatch.setattr(smf.submitted_edit, "mark_edited", lambda t, i, tr: state["marked"].append((i, tr)))
    return state


def test_upload_to_submitted_app_marks_edited(client, fakes):
    res = client.post(
        f"/sip-applications/me/milestone-files?application_id={APP_ID}",
        files={"file": ("m.pdf", io.BytesIO(b"%PDF-1.4 data"), "application/pdf")},
    )
...
def test_upload_window_closed_403(client, fakes, monkeypatch):
    ...
    monkeypatch.setattr(smf.submitted_edit, "load_editable_app", _raise)
    res = client.post(
        f"/sip-applications/me/milestone-files?application_id={APP_ID}",
        files={"file": ("m.pdf", io.BytesIO(b"%PDF-1.4 data"), "application/pdf")},
    )
